describe("AIPathOfBuilding RPC", function()
	...
	it("authenticates every request and dispatches protocol responses", function()
		...
		assert.are.equal(1, request.protocolVersion)
		assert.are.equal(string.rep("a", 32), request.sessionToken)
		table.insert(transport.incoming, json.encode({ jsonrpc = "2.0", protocolVersion = 1, id = id, result = { ok = true } }))
		assert.is_true(client:OnFrame())
	...
	it("dispatches notifications and rejects protocol mismatch", function()
		...
		assert.is_true(client:OnFrame())
		table.insert(transport.incoming, json.encode({ jsonrpc = "2.0", protocolVersion = 1, method = "run.progress", params = { phase = "search" } }))
		assert.is_true(client:OnFrame())
		assert.are.equal("search", phase)
		table.insert(transport.incoming, json.encode({ jsonrpc = "2.0", protocolVersion = 2, method = "run.progress", params = { } }))
