describe("AIPathOfBuilding Lua core", function()
	...
	it("captures a canonical sidecar snapshot without account state", function()
		...
		assert.are.equal(3, snapshot.schemaVersion)
		...
		assert.is_table(snapshot.mechanicProjection)
		assert.is_string(snapshot.mechanicProjectionFingerprint)
		assert.are.equal(snapshot.mechanicProjectionFingerprint, snapshot.mechanicProjection.fingerprint)
		assert.are.same({ "buff", "enchant", "scourge", "classRequirement", "implicit", "explicit", "crucible" }, snapshot.mechanicProjection.inventory.sections)
	...
	it("validates protocol v3 Trade, actor, and season actions", function()
		...
		assert.are.equal(3, normalized.version)
	...
	it("fails closed on unsupported action preconditions", function()
		...
		local invalid, invalidErr = BuildAction.Validate({
			version = 3, id = "guarded-object", kind = "config.setInput",
	...
	it("preserves advanced gem state when replacing links", function()
		...
		local action = {
			version = 3, id = "links", kind = "skills.replaceLinks",
	...
	it("rejects unsupported and cyclic actions", function()
		...
		local actions = {
			{ version = 3, id = "a", kind = "build.setProperty", dependsOn = { "b" }, payload = { property = "characterLevel", value = 90 } },
			{ version = 3, id = "b", kind = "build.setProperty", dependsOn = { "a" }, payload = { property = "characterLevel", value = 91 } },
		}
	...
	it("rejects and undoes passive allocations over the point budget", function()
		...
		local ok, err = BuildAction.Apply(fake, {
			version = 3, id = "over-budget", kind = "tree.setNode",
...
describe("AIPathOfBuilding transaction", function()
	...
	local function fakeBuild(level, hasConfig)
		local object = {
			...
			itemsTab = { items = { }, slots = { }, itemOrderList = { }, itemSetOrderList = { }, itemSets = { } },
		}
		if hasConfig then
			object.configTab = {
				activeConfigSetId = 1,
	...
	it("rolls back exact snapshot when commit apply fails after preflight", function()
		...
		local result = transaction:Apply({
			...
			actions = {
				{ version = 3, id = "level", kind = "build.setProperty", payload = { property = "characterLevel", value = 90 } },
				{ version = 3, id = "config", kind = "config.setInput", dependsOn = { "level" }, payload = { name = "enemyIsBoss", value = "Boss" } },
			},
		})
	...
	it("rolls back when commit metrics differ from preflight", function()
		...
		local result = transaction:Apply({
			id = "candidate", baseFingerprint = base.fingerprint,
			actions = { { version = 3, id = "level", kind = "build.setProperty", payload = { property = "characterLevel", value = 90 } } },
		})
		assert.is_false(result.ok)
		assert.is_true(result.rolledBack)
